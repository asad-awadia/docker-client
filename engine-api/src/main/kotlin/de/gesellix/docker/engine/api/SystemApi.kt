...
class SystemApi(basePath: String = defaultBasePath) : ApiClient(basePath) {
  ...
  @Suppress("UNCHECKED_CAST")
  @Throws(UnsupportedOperationException::class, ClientException::class, ServerException::class)
  fun systemEvents(since: String?, until: String?, filters: String?, callback: StreamCallback<SystemEventsResponse?>, timeoutMillis: Long /*= 24.hours.toLongMilliseconds()*/) {
    val localVariableConfig = systemEventsRequestConfig(since = since, until = until, filters = filters)

    val localVarResponse = requestStream<SystemEventsResponse?>(
      localVariableConfig
    )
        ...
        runBlocking {
          launch {
            withTimeout(timeoutMillis) {
              ...
              ((localVarResponse as SuccessStream<*>).data as Flow<SystemEventsResponse?>).collect { callback.onNext(it) }
              callback.onFinished()
            }
          }
        }
